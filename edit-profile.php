...
<div class="well-sm biale">
    <h1><?php echo $_LANG['labels']['edit-profile']; ?></h1> 
...

{
	...
	echo('<p>'.$_LANG[ 'edit-profile' ][ 'changed' ].'</p>');
...

{
...

	{
		...
		echo('<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><b>'.$_LANG['edit-profile'][ 'errors' ].'</b><br><ul class="list-unstyled" >');
	...
	echo('<dt class="h3">'. $_LANG[ 'labels' ][ '1st' ]. '</dt>');

	

	// Nick

	echo('<dt>'. $_LANG[ 'labels' ][ 'nick' ]. '</dt>');

	echo('<dd><input type="text" name="nick" value="'. $_USER[ 'nick' ] .'" class="nick" disabled></dd>');

	

	// First name

	echo('<dt><label for="register_05">'. $_LANG[ 'labels' ][ 'firstname' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><input type="text" id="register_05" value="'. $inputs['firstname'] .'" name="firstname" class="nick" required></dd>');

	

	// Surname

	echo('<dt><label for="register_06">'. $_LANG[ 'labels' ][ 'surname' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><input type="text" id="register_06" value="'. $inputs['surname'] .'" name="surname" class="nick" required></dd>');

	

	// Age

	echo('<dt><label for="register_07">'. $_LANG[ 'labels' ][ 'date' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><input type="date" id="register_07" value="'. $inputs['age'] .'" name="age" class="empty" required></dd>');

	

	// County

	echo('<dt><label for="register_09">'. $_LANG[ 'labels' ][ 'country' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><select name="country" class="empty" id="register_09">');
	...
	echo('<dt><label for="register_10">'. $_LANG[ 'labels' ][ 'city-code' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><input type="text" id="register_10" name="register_10" value="'. $inputs['city'] .'" class="empty city" required> <input type="text" name="register_11" value="'. $inputs['code'] .'" class="empty code" required></dd>');

	

	// Street

	echo('<dt><label for="register_12">'. $_LANG[ 'labels' ][ 'street' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd><input type="text" id="register_12" name="register_12" value="'. $inputs['street'] .'" class="empty" required></dd>');

	

	// Mail change

	echo('<dt><input type="checkbox" id="mail_01" name="mail_change"  value="true"');
	...
	echo('> <label for="mail_01">'. $_LANG[ 'labels' ][ 'mail-change' ]. '</label></dt>');

	echo('<dt'); if( !isset($_POST['mail_change']) || $_POST['mail_change'] != "true" ) { echo(' class="none"'); }

	echo('> <label for="register_04">'. $_LANG[ 'labels' ][ 'new-mail' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd'); if( !isset($_POST['mail_change']) || $_POST['mail_change'] != "true" ) { echo(' class="none"'); }

	echo('> <input type="text" value="'.$inputs['new_mail'].'" class="mail" id="register_04" name="register_04"><div class="status">&nbsp;</div>

		<input type="hidden" name="old_mail" value="'. $_USER[ 'email' ] .'"></dd>'); 

	

	// Password change

	echo('<dt><input type="checkbox" id="pass_01" name="pass_change" value="true"');
	...
	echo('> <label for="pass_01">'. $_LANG[ 'labels' ][ 'pass-change' ]. '</label></dt>');

	echo('<dt'); if( !isset($_POST['pass_change']) || $_POST['pass_change'] != "true" ) { echo(' class="none"'); }

	echo('><label for="register_02">'. $_LANG[ 'labels' ][ 'new-pw' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd'); if( !isset($_POST['pass_change']) || $_POST['pass_change'] != "true" ) { echo(' class="none"'); }

	echo('><input type="password" value="" class="pass1" id="register_02" name="register_02"><div class="status">&nbsp;</div><br>

	<div id="passwordStrengthDiv" class="is0"></div></dd>');

	echo('<dt'); if( !isset($_POST['pass_change']) || $_POST['pass_change'] != "true" ) { echo(' class="none"'); }

	echo('><label for="register_03">'. $_LANG[ 'labels' ][ 'new-repw' ]. '</label> <span class="star">*</span></dt>');

	echo('<dd'); if( !isset($_POST['pass_change']) || $_POST['pass_change'] != "true" ) { echo(' class="none"'); }

	echo('><input type="password" value="" class="pass1" id="register_03" name="register_03"><div class="status">&nbsp;</div></dd>');



	//-------------------------------------------------------------------------------------------------------------------------------------------

	echo('<dt class="margin-top h3">'. $_LANG[ 'labels' ][ '2nd' ]. '</dt>');

	

	// Newsletter

	echo('<dd><input type="checkbox" name="register_08" value="true" id="register_08"');
	...
	echo('> <label for="register_08">'. $_LANG[ 'labels' ][ 'newsletter' ]. '</label></dd>');

	

	// avatar

	echo('<dt><label for="register_13">'. $_LANG[ 'labels' ][ 'avatar' ]. '</label></dt>');

	echo('<dd><input type="file" id="register_13" class="nick" name="file" accept="image/jpeg,image/gif,image/png"></dd>');
...

	{
		...
		echo('<dt><input type="checkbox" id="register_14" name="register_14" value="true">  <label for="register_14">'. $_LANG[ 'labels' ][ 'del-avatar' ]. '</label></dt>');

		echo('<dd><img src="files/images/avatars/'.$_USER['avatar'].'" alt="avatar"></dd>');
...

	{
		...
		echo('<dd class="info">'. sprintf($_LANG[ 'edit-profile' ][ 'avatar-info' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'avatar' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'avatar' ])  .'</dd>');
	...
	echo('<dt class="margin-top h3">'. $_LANG[ 'labels' ][ '3rd' ]. '</dt>');
...

	{
		...
		echo('<dt>'. $_LANG[ 'labels' ][ 'refBy' ]. '</dt>');

		echo('<dd><input type="text" value="'. strtolower($l->getLoginById($_USER[ 'refId' ])) .'" class="nick" disabled></dd>');
	...
	echo('<dt>'. $_LANG[ 'labels' ][ 'refOur' ]. '</dt>');

	if( !empty( $_LANG[ 'edit-profile' ][ 'ref-info' ] ) )

	{

		echo('<dd>'. sprintf($_LANG[ 'edit-profile' ][ 'ref-info' ], $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'auth' ][ 'ref-bonus' ]) .'</dd>');

	}

	echo('<dd><input type="text" value="'. $_SETTINGS[ $_GLOBALS[ 'lang' ] ][ 'general' ][ 'url' ]. '/register/'.strtolower($_USER['nick']). '" id="ref-nick" class="nick" readonly></dd>');
...

	{
		...
		echo('<dt>'. $_LANG[ 'labels' ][ 'refCount' ]. '</dt>');

		echo('<dd><input type="text" value="'. $_USER['refCount']. '" class="nick" readonly></dd>');	
	...
	echo('<dd><input type="submit" name="edit_sub" value="'. $_LANG[ 'labels' ][ 'send-form' ]. '" class="btn btn-success"></dd>');
